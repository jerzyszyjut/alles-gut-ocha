...
\begin{description}[labelindent=0.5cm, labelsep=1em]
  ...
  \item[\texttt{chatbot.py}] Claude API integration.  Runs an agentic loop
    where Claude can call ten tools (data queries, parameter updates, HDX
    lookups) before returning a prose response.
  \item[\texttt{hdx\_client.py}] Rate-limited HTTP client (10 req/60 s) for
...
\begin{longtable}{p{3.8cm}p{4cm}p{6.5cm}}
  ...
  \texttt{fts\_requirements\_\allowbreak funding\_globalcluster\_global.csv} &
  ...
  INFORM &
  Multiple \texttt{.xlsx} files (2022--2025 monthly) &
  INFORM Severity Index.
  Used as a supplementary cross-check on severity signals. \\[4pt]

  UN OCHA COD &
  \texttt{cod\_population\_admin*.csv} &
  Sub-national population statistics (admin levels 0--4). \\
\end{longtable}
...
All live queries are rate-limited to 10 requests per 60 seconds.

\subsubsection*{Data merging pipeline}

The aggregate CSV (\code{aggregate.csv}) is produced by
\code{scripts/data/create\_big\_csv.py} and follows these steps:
...
#   HDX_APP_IDENTIFIER=...          (optional; base64(app_name:app_email))
...
Interactive API docs (Swagger UI) are at \url{http://localhost:8000/docs}.
...
If the aggregate CSV is missing or out of date, regenerate it:

\begin{lstlisting}[language=bash]
python scripts/data/create_big_csv.py
# Writes: data/aggregate.csv, data/aggregate_historical.csv
\end{lstlisting}
...
The table on the left displays all crises ranked by the neglect index using the
default weights ($S_w = 0.6$, $G_w = 0.4$).
...
\begin{itemize}[nosep]
  ...
  \item Click a column header to re-sort.
...
\begin{itemize}[nosep]
  ...
  \item The ghost position (original) and new position on the t-SNE chart.
\end{itemize}
